...
function pmf( x, options ) {
	...
	if ( isTypedArrayLike( x ) ) {
		...
		return pmf6( out, x, opts.p );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pmf4( x, opts.p, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pmf3( out, x, opts.p, opts.accessor );
		}
		return pmf2( out, x, opts.p );
	}
	return NaN;
} // end FUNCTION pmf()
