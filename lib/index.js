...
function pdf( x, options ) {
	...
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, opts.p );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, opts.p, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, opts.p, opts.accessor );
		}
		return pdf2( out, x, opts.p );
	}
	return NaN;
} // end FUNCTION pdf()
